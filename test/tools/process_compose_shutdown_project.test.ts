...
describe("process_compose_shutdown_project", () => {
  it("requires confirm=true", async () => {
    const client = createClientStub();

    const result = await handleToolCall(client, "process_compose_shutdown_project", {});
  ...
  it("shuts down project when confirmed", async () => {
    ...
    const result = await handleToolCall(client, "process_compose_shutdown_project", { confirm: true });
